...
class GraphMaker:
    ...
    ###parameters###
    lamda = 0.2
    NORMAL = True       # take NORMAL in account?
    ...
    def __init__(self):
        ...
        self.load_image('./RGB/' + self.ImageName + '.jpg', './Depth/' + self.ImageName + '_Depth.png')
        ...
        self.cfd_depth = None
        self.super_label = None                 #label of superpixels，0~5 for  0:LAB FG  1:LAB BG  2:depth FG  3:depth BG  4:normal FG  5:normal BG

    def computeSigma(self):
        # self.sigma1 = self.MAXIMUM
        # self.sigma2 = self.MAXIMUM
        # self.sigma3 = self.MAXIMUM
        ...
        for i in range(self.n_seg):
            for j in self.super_edge[i]:
                if j > i:
                    ...
                    # if self.sigma3 > (1 - self.cosine_sim(self.ave_normal[i], self.ave_normal[j])):
    ...
    def create_graph(self):
        ...
        #清空results
        if not os.path.exists("./results/" + self.ImageName):
            os.mkdir("./results/" + self.ImageName)
        for item in os.listdir("./results/" + self.ImageName):
            itemsrc = os.path.join("./results/" + self.ImageName, item)
            os.remove(itemsrc)

        self.swap(10)            # alpha-beta swap
        endtime = datetime.datetime.now()
        print("all: " + str((endtime - starttime).seconds))
    ...
    def swap(self, MaxIteration = 4):
        ...
        for i in range(MaxIteration):
            ...
            print(str(i + 1) + ", Energe：" + str(newEnergy))
    ...
    def smoothWeight(self, u, v, alpha = -1):
        ...
        if alpha == 0 or alpha == 1:
            ...
            weight1 = self.lamda * np.e**(-((1 - self.cosine_sim(self.ave_normal[v], self.ave_normal[u])) ** 2) / self.sigma3)
        ...
        if self.super_label[v] == 0 or self.super_label[v] == 1:
            ...
            weight2 = self.lamda * np.e**(-((1 - self.cosine_sim(self.ave_normal[v], self.ave_normal[u])) ** 2) / self.sigma3)
    ...
    def compute_alpha_beta_energy(self, alpha, beta, reflect):
        ...
        # print("本地项：" + str(energy))
        # print("其他本地项：" + str(energyout))
        # print("本地平滑项：" + str(energyadd))
        # print("其他平滑项：" + str(energysmoothout))
        ...
        for edge in self.edges:
            if self.super_label[reflect[edge[0]]] != self.super_label[reflect[edge[1]]]:
                energysmooth += edge[2]
        energy += (energysmooth)
        # print("平滑项：" + str(energysmooth))
        # for i in range(self.n_seg):
            # if self.super_label[i] == alpha:
            #     energy += cap_sink[i]
            #     for neighbor in self.super_edge[i]:
            #         if self.super_label[neighbor] != alpha:
            #             energy += self.smoothWeight(i, neighbor)
            # elif self.super_label[i] == beta:
            #     energy += cap_source[i]
            #     for neighbor in self.super_edge[i]:
            #         if self.super_label[neighbor] != alpha and self.super_label[neighbor] != beta:
    ...
    def getImage(self):
        ...
        cv2.imwrite("./results/" + self.ImageName + "/edgeImg.jpg", temp_img)
        ###get overlay###
        for x in range(0, len(self.superpixel_segment)):
            for y in range(0, len(self.superpixel_segment[0])):
        ...
        cv2.imwrite("./results/" + self.ImageName + "/backseeds.jpg", temp_img)

        ###get superpixels image###
        temp_img = self.superpixel_image.copy()
        cv2.imwrite("./results/" + self.ImageName + "/superpixel.jpg", temp_img)
        ###get LAB image###
        temp_img = self.LAB_map.copy()
        cv2.imwrite("./results/" + self.ImageName + "/LABmap.jpg", temp_img)
        ###get depth image###
        temp_img = self.depth_map / np.amax(self.depth_map) * 255
        cv2.imwrite("./results/" + self.ImageName + "/depthmap.jpg", temp_img)
        ###get normal image###
        if self.NORMAL:
            temp_img = self.normal_map * 255
            cv2.imwrite("./results/" + self.ImageName + "/normalmap.jpg", temp_img)
        ...
        cv2.imwrite("./results/" + self.ImageName + "/confident_LAB.jpg", temp_img)
        ...
        cv2.imwrite("./results/" + self.ImageName + "/confident_depth.jpg", temp_img)
        ...
        if self.NORMAL:
            ...
            cv2.imwrite("./results/" + self.ImageName + "/confident_normal.jpg", temp_img)

        ###get results###
        temp_img = cv2.addWeighted(self.image, 0.2, self.segment_overlay, 0.8, 0.1)
        cv2.imwrite("./results/" + self.ImageName + "/segment.jpg", temp_img)
        ###results with cues###
        self.segment_overlay = np.zeros_like(self.image)
        for x in range(0, len(self.superpixel_segment)):
            for y in range(0, len(self.superpixel_segment[0])):
        ...
        temp_img = cv2.addWeighted(self.image, 0.2, self.segment_overlay, 0.8, 0.1)
        cv2.imwrite("./results/" + self.ImageName + "/segment_cues.jpg", temp_img)
        ...
        cv2.imwrite("./results/" + self.ImageName + "/ave_depth.jpg", temp_img)
    ...
    def get_seg_now(self, path):                #  get current seg
        ...
        cv2.imwrite("./results/" + self.ImageName + "/" + path + ".jpg", temp)
